...
class Dataset(object):
    ...
    def __init__(self, name, attr0_name, attr1_name, npzfile, seed=0, use_attr=False, load_on_init=True,budget = 1):
        self.name = name
    ...
    def make_validation_set(self):
        if not 'valid' in self.tensors['x']:
            for t in self.tensor_names:
                ...
                #make the budgeted training set
                self.tensors[t]['train'] = self.tensors[t]['train'][:int(self.budget*len(self.tensors[t]['train']))]
