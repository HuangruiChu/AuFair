...
class Trainer(object):
    def __init__(self, model, data, batch_size=BATCH_SIZE, learning_rate=LEARNING_RATE, sess=None, logs_path=LOG_PATH, \
        ...
        #create optimizer
        self.train_opt = tf.compat.v1.train.AdamOptimizer(learning_rate=LEARNING_RATE)  
    ...
    def train(self, n_epochs, patience):
        ...
        for epoch in range(n_epochs):
            ...
            if not self.logs_path is None:
                summary_writer = tf.compat.v1.summary.FileWriter(self.logs_path, self.sess.graph)
                summary_writer = tf.compat.v1.summary.FileWriter(self.logs_path, self.sess.graph)
    ...
    def test(self):
        ...
        return error_rate,abs(tpr0-tpr1)
